...
export function transformWhereString(
  ...
  switch (operator) {
    ...
    case QueryOperators.MORE_THAN:
      return MoreThan(value);
    case QueryOperators.LESS_THAN_OR_EQUAL:
      return LessThanOrEqual(value);
    case QueryOperators.MORE_THAN_OR_EQUAL:
      return MoreThanOrEqual(value);
    case QueryOperators.NOT_CONTAIN:
      return Not(ILike(`%${value}%`));
    case QueryOperators.NOT_END_WITH:
      return Not(ILike(`%${value}`));
    case QueryOperators.NOT_START_WITH:
      return Not(ILike(`${value}%`));
 ...
 * ```
 */
export function WhereStringTranform(): PropertyDecorator {
  return (t, p) => {
    Transform(({ value }) => {
